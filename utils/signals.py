...
def add_signals(ticker_df: pd.DataFrame):
        ...
        ticker_df["bb_signal"] = np.where(ticker_df["close"] < ticker_df["bb_lower"], 1, np.where(ticker_df["ticker_df"] > ticker_df["bb_uper"], -1, 0))
...
def add_signal_weight(signal_df: pd.DataFrame, signal_column_name: str ,singal_weight_column: str):
    ...
    for index, row in signal_df.iterrows():      
        if row[signal_column_name] == 1:         
            ...
            signal_df.loc[index, singal_weight_column] = prev_sig_weight * 0.5
