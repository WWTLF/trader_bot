...
def decide(current_day: datetime.datetime):
    ...
    for stock_name in stocks:
        ...
        extra_feature_repo.save(ExtraFeature(stock_name, current_day + relativedelta(days=lstm_offset), 'lstm_pred_t3_close', lstm_pred_t3_close))

        ts_mixer_t3_close, _ = tsmixer_predictor.predict(current_day)
        extra_feature_repo.save(ExtraFeature(stock_name, current_day + relativedelta(days=ts_offset), 'ts_mixer_t3_close', ts_mixer_t3_close))
        ...
        extra_feature_repo.save(ExtraFeature(stock_name, current_day, 'not_trust', not_trust))
        ...
        extra_feature_repo.save(ExtraFeature(stock_name, current_day + relativedelta(days=3), 'mid_pred_t3_close', mid_pred_t3_close))
        ...
        extra_feature_repo.save(ExtraFeature(stock_name, current_day+relativedelta(days=3), 'pct_pred_close', pct_pred_close))
        extra_feature_repo.save(ExtraFeature(stock_name, current_day+relativedelta(days=3), 'filtered_mid_price_t3', filtered_mid_price_t3))
        extra_feature_repo.save(ExtraFeature(stock_name, current_day+relativedelta(days=3), 'filtered_covariance_t3', filtered_covariance_t2))


        # Простое правило, выдающее предварительный сигнал
        if filtered_mid_price_t3[0,0] > filtered_mid_price_t0:
            new_signal = 1
        elif filtered_mid_price_t3[0,0] < filtered_mid_price_t0:
            new_signal = -1
    
        
        prev_signal = extra_feature_repo.get_one_by_ticker_and_date(stock_name, current_day + relativedelta(days=-1), 'signal')
        ...
        extra_feature_repo.save(ExtraFeature(stock_name, current_day, 'signal', new_signal))

        # not_trust = 0.0 # Для предварительно обучения всегда 0, затем нужно закомментировать
        is_close, final_signal = ds.decide(current_day, new_signal, signals_df.loc[current_day]['close'], 1, not_trust)
        # result_df.iloc[i, result_df.columns.get_loc('is_close')] = is_close
        extra_feature_repo.save(ExtraFeature(stock_name, current_day, 'is_close', is_close))
        # result_df.iloc[i, result_df.columns.get_loc('final_signal')] = final_signal
        extra_feature_repo.save(ExtraFeature(stock_name, current_day, 'final_signal', final_signal))
